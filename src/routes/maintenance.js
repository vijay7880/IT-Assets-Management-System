...
const router = Router();
const fields = ['RDTag_No','Asset_Name','Asset_Category','Asset_Model','Asset_Serial_No','Problem','Action','Status'];
...
router.get('/search-main', async (req, res, next) => { try { const value = `%${req.query.name || ''}%`; const [Maint] = await db.query('SELECT * FROM Maintenance WHERE RDTag_No LIKE ? OR Asset_Name LIKE ? OR Asset_Category LIKE ?', [value, value, value]); res.render('Maintenance', { Maint }); } catch (e) { next(e); } });
...
router.post('/add_maintenance', allowRoles('manager', 'admin'), async (req, res, next) => { try { await db.query(`INSERT INTO Maintenance (${fields.join(',')}) VALUES (${fields.map(() => '?').join(',')})`, valuesFrom(req.body)); await db.query('UPDATE Assets SET Status = ? WHERE Serial_No = ?', ['In Repair', req.body.Asset_Serial_No]); res.send('<script>alert("Submit"); window.location.href = "/add_main";</script>'); } catch (e) { next(e); } });
...
router.get('/view_main/:RDTag_No', async (req, res, next) => { try { const [result] = await db.query('SELECT * FROM Maintenance WHERE RDTag_No = ?', [req.params.RDTag_No]); res.render('view_main', { Maint: result[0] }); } catch (e) { next(e); } });
