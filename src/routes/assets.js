...
router.get('/asset_view/:RDTag_No', async (req, res, next) => { try { const [result] = await db.query('SELECT * FROM Assets WHERE RDTag_No = ?', [req.params.RDTag_No]); res.render('Asset_view', { Assets: result[0] }); } catch (e) { next(e); } });
