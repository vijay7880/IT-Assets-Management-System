...
router.get('/search-ass', async (req, res, next) => { try { const value = `%${req.query.name || ''}%`; const [Assigned] = await db.query('SELECT * FROM Assigned WHERE RDTag_No LIKE ? OR Asset_Name LIKE ? OR Asset_Category LIKE ? OR Department LIKE ?', [value, value, value, value]); res.render('Assigned', { Assigned }); } catch (e) { next(e); } });
